...
import { rotatingHomeBackgrounds } from "../home-backgrounds";
...
const HOME_BACKGROUND_ROTATE_INTERVAL_MS = 60_000;
...
export function PublicLayout({
  ...
  const homeBackgrounds = useMemo(() => {
    if (isHomePage && rotatingHomeBackgrounds.length > 0) {
      return rotatingHomeBackgrounds;
    }

    return [siteConfig.theme.fuwari.homeBg];
  }, [isHomePage, siteConfig.theme.fuwari.homeBg]);

  useEffect(() => {
    setCurrentBackgroundIndex(0);

    if (!isHomePage || homeBackgrounds.length <= 1) {
      return;
    }

    const timer = window.setInterval(() => {
      setCurrentBackgroundIndex((previousIndex) => {
        return (previousIndex + 1) % homeBackgrounds.length;
      });
    }, HOME_BACKGROUND_ROTATE_INTERVAL_MS);

    return () => window.clearInterval(timer);
  }, [homeBackgrounds, isHomePage]);

  const currentBackground =
    homeBackgrounds[currentBackgroundIndex] ?? siteConfig.theme.fuwari.homeBg;
